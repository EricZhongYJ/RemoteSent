...
% qt = qv+qr;
% 
% u = load('Data2D/UYHalf_99.dat');
% movies = size(u,1)/(nx*m)
% u = reshape(u,nx,m,movies); %This are already in m/s
% 
% w = load('Data2D/WYHalf_99.dat');
% movies = size(w,1)/(nx*m)
% w = reshape(w,nx,m,movies); %This are already in m/s
% 
% figure(1)
% hold on
% Freq = 1;
...
% u_frame(:,:) = u(:,:,mov); 
% w_frame(:,:) = w(:,:,mov); 
% quiver(x(1,1:Freq:nx),z(1,1:Freq:m),transpose(u_frame(1:Freq:nx,1:Freq:m)),transpose(w_frame(1:Freq:nx,1:Freq:m)))
% %caxis([-2 2])
% axis([0  Lx 0 Lz])
% 
% %area = sum(sum(dx*dy*dz*qt(:,:,mov)))
% print('QtPrime2DT10Min','-depsc',figure(1))
% 
% % stop
...
qv_mov = load('Data3D/Qv_03.dat');
size(qv_mov)/(nx*ny*m)
qv_mov=reshape(qv_mov,nx,ny,m);

qr_mov = load('Data3D/Qr_03.dat');
size(qr_mov)/(nx*ny*m)
...
print('QtPrime3DT36min','-depsc',figure(1))
%% stop
